...
/*
 * bootloader file:
 * primary code unit of microvisor, is immediately called by Reset_Handler after
 * board reboot. Performs all necessary initialization tasks (UART, MPU, etc.)
...
// manage
__attribute__((section(".microvisor-nopri"))) int
custom_printf(const char *format, ...) {
  va_list args;
 ...
 */
__attribute__((section(".microvisor-nopri"))) static void
start_app(unsigned int sp, unsigned int pc) {
  __asm__(
...
/**
 * Set CCR.NONBASETHRDENA to 1 in order to disable the integrity check on
 * exception return. This is done to prevent the ARMv7 architecture from
 * interfearing with the process of interrupt deprioritization in the event of 2
 * interrupts (A and B with B having higher priority than A) where interrupt B
 * could preempt the execution of A during the exception catching routine
 * immediately before interrupts are disabled.
 */
void Disable_Exception_Integrity_Check() {
  __asm__("ldr r0, =0xE000ED14\n" // address of Configuration and Control
                                  // Register (CCR)
          "ldr r1, [r0]\n"
          "orr r1, #1\n"
          "str r1, [r0]\n");
...
void boot(void) {
  ...
  /* Configure MPU in order to execute the CA (Client Application) with only
   * permissions in its dedicated areas */
  Configure_MPU();
  Disable_Exception_Integrity_Check();
  Initialize_Virtual_IPSR();

  // Erase both of the flash memory area dedicated to secure storage (?) of the
  // TAs
  flash_erase(1);
  flash_erase(2);
