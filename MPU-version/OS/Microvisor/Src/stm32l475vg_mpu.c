 ...
 * RM0434 - Multiprotocol wireless 32-bit MCU Arm®-based Cortex®-M4 with FPU,
 * Bluetooth® Low-Energy and 802.15.4 radio solution
 */
void Configure_MPU(void) {
  ...
  MPU_InitStruct.AccessPermission =
      MPU_REGION_PRIV_RW_URO; // privileged read+write, unprivileged read only
  // the subregions for the two TAs (specifically the 2/3/4/5 - each of 128KB)
  // should be disabled value if binary: 0b00011110 = 0x1E
  ...
  MPU_InitStruct.Size =
      MPU_REGION_SIZE_128KB; // It's 32kb but starting after the Bootloader
                             // reserved RAM whose region will have higher
                             // priority
  MPU_InitStruct.DisableExec = MPU_INSTRUCTION_ACCESS_DISABLE;
  ...
  MPU_InitStruct.DisableExec = MPU_INSTRUCTION_ACCESS_DISABLE;
  MPU_InitStruct.AccessPermission = MPU_REGION_PRIV_RW;
  // MPU_InitStruct.AccessPermission = MPU_REGION_FULL_ACCESS;	// privileged
  // read+write, unprivileged read only
  MPU_InitStruct.SubRegionDisable = 0x00;
  /* Cache properties and shareability  */
  /* Device type: Device */
  MPU_InitStruct.TypeExtField = MPU_TEX_LEVEL2;
  ...
  /* Enable MPU */
  /* MPU_PRIVILEGED_DEFAULT enable the backgroud region for priviledged
   * processes */
...
/**
 * Reconfigure the MPU to enable the execution of TA1 or TA2 based on the
 * TA_number parameter
 *
 * Higher regions number have more priority in the event of multi-matching,
 * this behavior is used to cover the regions in a coarse-grained approach
...
void Reconfigure_MPU(int TA_number) {
  ...
  // but it is not as there is a specific region for the bootloader that has
  // priority and regulates the access the subregions for the other TA (each of
  // 128KB) shold be disabled the ones of fortified app/boot no priority
  // (specifically the 6/7/8) should be disabled but can not because the region
  // boot no priority is used to perform the jump to the TAs and it is part of
  // the last region togheter with the fortified app NOTE: Subregion Disable -
  // 0: subregion enabled; - 1: subregion disabled NOTE: Subregions count is
  // from 1 in this case (not 0)
  ...
  MPU_InitStruct.Number = MPU_REGION_NUMBER2;
  MPU_InitStruct.BaseAddress = 0x10000000;
  MPU_InitStruct.Size = MPU_REGION_SIZE_32KB; // cannot create region of size
                                              // 96KB, do immediately bigger one
  MPU_InitStruct.DisableExec = MPU_INSTRUCTION_ACCESS_DISABLE;
  MPU_InitStruct.AccessPermission = MPU_REGION_FULL_ACCESS;
...
void MPU_Violation_Handler(unsigned int *auto_frame,
                           unsigned int *manual_frame) {
  unsigned int STK_Realign =
      __get_xPSR() & 0x100U; // Bit 9 of xPSR indicate stack realingment
                             // operation during exception entry

  /* Compute significant register values */
  unsigned int PreEntry_SP =
      STK_Realign
          ? (unsigned int)auto_frame + 0x24
          : (unsigned int)auto_frame +
                0x20U; // Recover original value of SP (before exception entry)
  ...
  perma_storage_store_error(ERROR_MPU_VIOLATION, buffer,
                            ERROR_MPU_VIOLATION_DATA_LEN);
