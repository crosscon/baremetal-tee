...
#define WIFI_MAX_ATTEMPTS 1000	// Maximum number of WIFI_ReceiveData() calls before giving up
...
/**
  ...
  * @retval Operation status WIFI_STATUS_OK if the connection was opened succesfully, WIFI_STATUS_ERROR otherwise
  */
WIFI_Status_t wifi_wrapper_open_client_connection(uint32_t socket, uint8_t *ipaddr, uint16_t port, uint16_t local_port);

/**
  * @brief  Closes previously opened TCP connection
  * @param socket: socket (0-3) selected
  * @retval Operation status WIFI_STATUS_OK if the connection was closed succesfully, WIFI_STATUS_ERROR otherwise
  */
...
/**
  ...
  * - only partial data arrived and attempts exhausted (function returns WIFI_STATUS_ERROR)
  ...
  * @retval Operation status WIFI_STATUS_OK when all data requested was recevied, WIFI_STATUS_ERROR when partial data recevied or error
  */
WIFI_Status_t wifi_wrapper_receive_data(uint32_t socket, uint8_t *pdata, uint16_t Reqlen);
...
/**
  ...
  * - only partial data has been sent and attempts exhausted (function returns WIFI_STATUS_ERROR)
  * - error encountered during receive (function returns WIFI_STATUS_ERROR)
  * @param socket: socket (0-3) selected
  ...
  * @retval Operation status WIFI_STATUS_OK when all data requested was recevied, WIFI_STATUS_ERROR when partial data recevied or error
  */
WIFI_Status_t wifi_wrapper_send_data(uint32_t socket, uint8_t *pdata, uint16_t Reqlen);
