...
/*
 * All the following functions are the actual implementation of the TEE Core API
 * functions They are called by the TA functions using these steps:
 * - call the entry point defined in the tee_core_api files (accessible by the
 * TA)
 * - copy the parameters into the CPU registers (from r4 to r11)
 * - invoke the SVC instruction to switch to the secure world (with a particular
 * SVC number)
 * - based on the SVC number, the secure world will call the corresponding
 * internal function that map the TEE Core API
 *
 * The return value of the internal functions is stored in the r0 register and
 * is returned to the TA
 *
 * The internal functions are defined in this file and are called by the secure
 * world
 */
 ...
 * Blocks, where each block contains information about the size and a pointer to
 * the next block
 *
 ...
 * BigInt numbers are used to represent large numbers that cannot be stored in a
 * standard data type BigInt numbers are needed for cryptographic operations
 *
 ...
 *
 * The following functions are used to manage two types of objects: transient
 * and persistent Transient objects are stored in the stack and heap memory of
 * the TA Persistent objects are stored in the secure storage memory of the TA
 *
 * AllocateTransientObject: allocate a transient object in the memory
 * (attributes and buffer) PopulateTransientObject: populate the attributes of a
 * transient object InitRefAttribute: initialize a reference attribute
 ...
 *
 * More information about the functions can be found in the TEE Core API
 * documentation
 */

TEE_Result internal_TEE_AllocateTransientObject(
...
TEE_Result internal_TEE_ReadObjectData(TEE_ObjectHandle object, void *buffer,
                                       size_t size, size_t *count,
                                       uint8_t ta_num);

TEE_Result internal_TEE_WriteObjectData(TEE_ObjectHandle object, void *buffer,
                                        size_t size, uint8_t ta_num);
 ...
 *
 * The following functions are used to manage cryptographic operations according
 * to the TEE Core API
 *
 ...
 * SetOperationKey2: set two keys for a cryptographic operation (NOT
 * IMPLEMENTED)
 *
 * More information about the functions can be found in the TEE Core API
 * documentation
 ...
 *
 * The following functions are used to generate random values and keys for
 ...
 * AES algorithm (with different modalities like CTR, GCM, ...) is used and
 * supported for encryption and decryption
 *
 ...
 *
 * The following functions are used to sign a given message and to verify a
 * signature ECDSA algorithm (with SHA) is supported for asymmetric operations
 *
...
void internal_TEE_DigestUpdate(TEE_OperationHandle operation, void *chunk,
                               size_t chunkSize, uint8_t ta_num);
