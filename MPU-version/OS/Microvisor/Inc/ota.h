...
/**
 * \brief			Computes size of fortified application by
 * scrolling backwards through flash memory until first non-empty (non
 * 0xffffffff) word is found.
 * \return			Size of current fortified application in bytes
 * (0 if not application is present).
 */
uint32_t ota_compute_firmware_size();

/**
 * \brief			Computes sha256 of flash memory assigned to user
 * application.
...
/**
 * \brief			Checks remote server for Microvisor update and
 * performs it if available. Attention, updating the microvisor will wipe the
 * user application currently installed.
...
/**
 * \brief			Provides errors stored in permanent storage to
 * remote server.
 * \param ctx		sec_comm context (must be already initialized)
 * \return			OTA_OK or some error.
 */
...
/**
 * \brief			Checks update availablility and performs
 * firmware update, writes new firmware to flash memory, verifies correct
 * update, writes new firmware to flash.
 * \param ctx		sec_comm context (must be already initialized)
 * \return			OTA_OK or some error.
 */
