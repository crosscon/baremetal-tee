...
typedef struct {
  volatile uint8_t activation_code
      [PERMA_STORAGE_ACTIVATION_CODE_LEN]; /* Activation code downloaded from
                                              activation server on first boot */
  volatile uint32_t firmware_size;         /* Size of fortified application */
  volatile uint32_t
      error_id; /* ID of occurred error (0xffffffff no error occurred) */
