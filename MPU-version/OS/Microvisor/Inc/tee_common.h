...
/**
 * Constants used to store information about the TAs and CA in the system,
 * namely:
 * - max number of TAs (2)
...
#define TA_HEAP_SIZE                                                           \
  1024 * 10 // For the moment, each TA has a dedicated heap of 10KB
#define CORE_HEAP_SIZE                                                         \
  1024 * 8 // For the moment, the TEE core has a dedicated heap of 8KB
 ...
 * - the base address where the secure storage area starts for each TA (based on
 * memory map)
 ...
 */
#define MCU_FLASH_PAGE_SIZE                                                    \
  2048 // Page or sector size, please check the MCU datasheet
#define TA1_SEC_STR_BASE_ADDR                                                  \
  0x0805C000 // Base address of the secure storage area for TA1
#define TA2_SEC_STR_BASE_ADDR                                                  \
  0x0809C000 // Base address of the secure storage area for TA2
#define TA1_PAGE_COUNT                                                         \
  8 // Number of pages used for the secure storage area for TA1
#define TA2_PAGE_COUNT                                                         \
  8 // Number of pages used for the secure storage area for TA2
#define TA1_PAGE_INDEX                                                         \
  184 // Page index where the secure storage area for TA1 starts
#define TA2_PAGE_INDEX                                                         \
  312 // Page index where the secure storage area for TA2 starts
#define TA1_FLASH_SIZE                                                         \
  (MCU_FLASH_PAGE_SIZE *                                                       \
   TA1_PAGE_COUNT) // Size of the secure storage area for TA1
#define TA2_FLASH_SIZE                                                         \
  (MCU_FLASH_PAGE_SIZE *                                                       \
   TA2_PAGE_COUNT) // Size of the secure storage area for TA2

// This file contains the definition for all the types and constants used in the
// Microvisor and the TAs following the global platform standard

/*	----------------------------------Constants and Macro Functions
 * ----------------------------------------	*/

/**
 * Defintion of a custom function that encapsulate the TEE parameter types from
 * the four parameters values into a single one
 */
#define TEE_PARAM_TYPES(t0, t1, t2, t3)                                        \
  ((t0) | ((t1) << 4) | ((t2) << 8) | ((t3) << 12))

/**
 * Defintion of a custom function to get the parameters type from the single TEE
 * parameter value
 */
 ...
 * uses internal buffers and data structures that were difficult to control and
 * manage
 *
...
#define ERR_MSG(fmt, ...)                                                      \
  do {                                                                         \
    printf("ERROR INSIDE TEE: %s:%d:%s(): " #fmt "\r\n", __FILE__, __LINE__,   \
...
/**
 * Definition of the function codes used to call the TA CLIENT APIs in the SVC
 * handler
 *
...
/**
 * Definition of the function codes used to call the TA CORE APIs in the SVC
 * handler
 *
...
// --------------  TEEC types definition (datastructures mainly used by the
// client application) --------------
...
/**
 * Type definition for the TEEC_Parameter structure that contains the parameters
 * to be passed to the TA The parameters can be of the following types:
...
// Type definition for the TEE_Attribute structure for storing the attributes of
// an object As the structure is an union, it can store either a reference or
// values
typedef struct {
...
/**
 * Internal type definition for storing all the information related to a request
 * to a specific TA
 */
typedef struct {
  void *params[4];      // Stores the TEEC_Parameter to be passed to the TA
  uint32_t paramTypes;  // Parameter types (single number, to be decoded with
                        // TEE_PARAM_TYPE_GET)
 ...
 *
 * The object data is stored in the buffer field (after reading it from the
 * secure storage in case of persistent objects) The object attributes are
 * stored in the attrs field Object ID, an identification used for each object,
 * is stored in the obj_id field The flags field is used to define if the
 * payload is readable or writable The obj_type field is used to define the type
 * of the object (AES, ECDSA, HMAC, etc.) The obj_storage_type field is used to
 * define the storage type of the object (transient or persistent) The key_len
 ...
 * This structure is used to store the operation attributes and data (different
...
#define TEE_ALG_ECDSA_P256                                                     \
  TEE_ALG_ECDSA_SHA256 // Replaced with TEE_ALG_ECDSA_SHA256, same value
