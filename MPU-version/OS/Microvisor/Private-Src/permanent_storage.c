...
/**
 * \brief Erases all the content of the permanent storage (1 page only on the flash memory)
*/
uint32_t perma_storage_wipe() {
	...
	FLASH_EraseInitTypeDef erase_confige = {
		  ...
		  1,
		  (((uint32_t) &__perma_storage_start__) - (uint32_t) &__flash_boot_start__) / FLASH_PAGE_SIZE,
		  1
	};
	uint32_t page_error;
...
/**
 * \brief Update data in firmware storage while preserving content previously written
...
uint32_t perma_storage_update(uint8_t* data, uint8_t len, uint32_t address) {
	...
	uint8_t perma_offset = (uint8_t) (address - (uint32_t) &__perma_storage_start__);
	...
	for(uint32_t i = 0; i < FLASH_PAGE_SIZE; i += 8) {
		if(HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, ((uint32_t) &__perma_storage_start__ + i), *buffer_ptr) != HAL_OK)
			return PERMA_STORAGE_FLASH_ERROR;
		buffer_ptr++;
	}	
...
uint32_t perma_storage_set_firmware_size(uint32_t new_size) {
	return perma_storage_update((uint8_t *) &new_size, 4, (uint32_t) &(PERMA_STORAGE->firmware_size));
}
...
uint32_t perma_storage_store_error(uint32_t error_id, uint32_t* error_data, uint32_t error_data_len) {
	uint32_t status;
	// Store error id
	status = perma_storage_update((uint8_t *) &error_id, 4, (uint32_t) &(PERMA_STORAGE->error_id));
	if(status != 0)
		return status;
	// Store error data
	return perma_storage_update((uint8_t *) error_data, (uint8_t) error_data_len << 2, (uint32_t) &(PERMA_STORAGE->error_data));
}
...
uint32_t perma_storage_clear_error() {
	...
	return perma_storage_update((uint8_t *) &empty, 4, (uint32_t) &(PERMA_STORAGE->error_id));
}
...
uint32_t perma_storage_set_activation_code(uint8_t* ac) {
	return perma_storage_update(ac, PERMA_STORAGE_ACTIVATION_CODE_LEN, (uint32_t) &(PERMA_STORAGE->activation_code));
}