...
#define ASSERT_INOUT(ta_num, buf, size)                                        \
  if (check_mem_ownership(ta_num, buf, size) != true)
#define ASSERT_OUT(ta_num, buf, size) ASSERT_INOUT(ta_num, buf, size)
#define ASSERT_IN(ta_num, buf, size) ASSERT_INOUT(ta_num, buf, size)

// A set of assertions for the [inbuf], [outbuf], [inoutbuf] annotations.
#define ASSERT_INOUTBUF(ta_num, buf, size)                                     \
 ...
 *
 * The heap memory is allocated as static array and is managed with a list of
 * Blocks Each block contains information about the size and a pointer to the
 * next block
 *
 * Each TA has a dedicated heap memory area of 10 KB, allocated as uint32_t
 * array uint32_t is used to ensure that every type of variable (with different
 * types) can be stored in the heap memory. The heaps are placed in different
 * memory sections using the (heap_TA1 and heap_TA2) using the linker script
 * file
 *
 * This memory can be allocated using the TEE_Malloc function and freed using
 * the TEE_Free function The Core will instead use internal_TEE_Malloc and
 * internal_TEE_Free functions, passing 0 as the TA number
 */
__attribute__((section(
    ".heap_TA1"))) static uint32_t heapTA1[TA_HEAP_SIZE / sizeof(uint32_t)];
__attribute__((section(
    ".heap_TA2"))) static uint32_t heapTA2[TA_HEAP_SIZE / sizeof(uint32_t)];
__attribute__((section(
    ".heap_core"))) static uint32_t heapCore[CORE_HEAP_SIZE / sizeof(uint32_t)];
...
// These will be used to keep track of the various objects and operations
// created by the TAs
__attribute__((section(".ram-boot")))
__TEE_ObjectHandle *registeredObjects[MAX_HANDLES] = {0};
__attribute__((section(".ram-boot")))
__TEE_OperationHandle *registeredOperations[MAX_HANDLES] = {0};

/**
 * @brief Checks if the segment identified by a base pointer and a size is
 * completely contained in an interval starting at a start address and ending at
 * a end address (inclusive).
 *
 * @param base The base address of the segment.
 * @param size The size of the segment starting from base.
 * @param start The start address of the interval (inclusive).
 * @param end The end address of the interval (inclusive).
 *
 * @return true if the segment is contained in the interval, false otherwise.
 */
static inline bool is_segment_contained(void *base, size_t size, void *start,
                                        void *end) {
  // Checked this way to avoid numeric overflows. (end - base is a positive
  // quantity).
  return base >= start && base <= end && size <= end - base;
}

/**
 * @brief   Check if the memory address is owned by the TA (in its memory area).
 *          This check corresponds to the [in], [out], [inout] annotations in
...
static bool check_mem_ownership(uint8_t ta_num, void *buffer, size_t size) {
  if (buffer == NULL) {
    if (size == 0) {
      // [in] buffers can be NULL if the size is 0.
      return true;
    } else {
  ...
  if (ta_num == TA1_NUM) {
    if (is_segment_contained(buffer, size, TA1_MEMORY_START_ADDR,
                             TA1_MEMORY_END_ADDR) != true) {
      return false;
    }
  } else if (ta_num == TA2_NUM) {
    if (is_segment_contained(buffer, size, TA2_MEMORY_START_ADDR,
                             TA2_MEMORY_END_ADDR) != true) {
      return false;
    }
  } else {
    return false; // Invalid TA number
  }
  return true;
}

/**
 * @brief   Check if the memory address is accessible by the TA (in its memory
 * area or shared memory). This check corresponds to the [inbuf], [outbuf],
 * [inoutbuf] annotations in the GP specification.
 * @param   ta_num TA number (0 for TEE Core, 1 for TA1, 2 for TA2)
 * @param   buffer Pointer to the memory area to check
 * @param   size Size of the memory area to check
 * @return  true if the memory area is within the range, false otherwise
...
/**
 * @brief Check if the memory address is within the range of the heap memory for
 * the given ta_num.
 * @param ta_num TA number (0 for TEE Core, 1 for TA1, 2 for TA2)
 * @param buffer Pointer to the memory area to check
 * @param size Size of the memory area to check
...
static uint8_t check_heap_ownership(uint8_t ta_num, void *buffer, size_t size) {
  ...
  if (is_segment_contained(buffer, size, start, end) != true) {
...
/**
 *  @brief  Returns the base pointer for the area associated with the given
 * block.
 *
 *  @param  block The block the area is owned by.
 *  @return The area of the block.
 */
static inline void *area_of(Block *block) { return (void *)(block + 1); }

/**
 *  @brief  Check if a block correctly refers to the heap data or has been
 * tampered. Both the block content and the referenced area are checked.
 *  @param ta_num TA number (0 for TEE Core, 1 for TA1, 2 for TA2)
 ...
 */
static uint8_t check_operation_ownership(uint8_t ta_num,
 ...
 *   @return true if the object or operation was successfully populated, false
 * otherwise
 */
static bool populate_object(__TEE_ObjectHandle *temp_obj, uint8_t ta_num) {
  for (int i = 0; i < MAX_HANDLES; i++) {
    if (registeredObjects[i] == NULL) {
      registeredObjects[i] = temp_obj;
      temp_obj->ta_num = ta_num;
      return true;
    }
  }
  return false;
...
/**
 * @brief Allocates a new object handle (transient or persistent) and quickly
 * initializes it. Further initialization could be needed. The object is
 * allocated, registered and the buffer is allocated.
 *
 * @param ta_num The TA this object is associated to.
 * @param obj_id The object identifier.
 * @param size The size of the buffer.
 * @param flags The accessibility flags.
 * @param obj_storage_typ The type of this object (persistent or transient).
 *
 * @return The newly allocated object, NULL in case of errors.
 */
static __TEE_ObjectHandle *allocate_object_handler(uint8_t ta_num,
                                                   uint32_t obj_id, size_t size,
                                                   uint32_t flags,
                                                   uint8_t obj_storage_type) {
  __TEE_ObjectHandle *obj = (__TEE_ObjectHandle *)internal_TEE_Malloc(
      sizeof(__TEE_ObjectHandle), TEE_MALLOC_NO_FILL, CORE_NUM);
  if (obj == NULL) {
    return NULL;
  }

  if (populate_object(obj, ta_num) != true) {
    internal_TEE_Free(obj, CORE_NUM);
    return NULL;
  }

  if (size != 0) {
    obj->buffer = internal_TEE_Malloc(size, TEE_MALLOC_NO_FILL, CORE_NUM);
    if (obj->buffer == NULL) {
      free_object(obj);
      internal_TEE_Free(obj, CORE_NUM);
      return NULL;
  ...
  obj->obj_id = obj_id;
  obj->flags = flags;
  obj->obj_storage_type = obj_storage_type;
 ...
 */
static void deallocate_object_handler(__TEE_ObjectHandle *obj) {
  free_object(obj);
  if (obj->buffer != NULL) {
...
// Set the correct values for the heap memory of each TA when they are first
// used
  ...
  while (curr != NULL) {
    ...
    if ((curr->free) && (block_size >= size)) {
      ...
      if (hint == TEE_MALLOC_FILL_ZERO) {
        // NOTE: it is necessary to cast to void* otherwise pointer arithmetic
...
void internal_TEE_Free(void *buffer, uint8_t ta_num) {
  ...
  while (check_block_ownership(ta_num, curr)) {
    ...
    if (curr->free) {
      ...
      // First check if the block is a valid block to avoid leaking memory
      // information.

      // Otherwise a TOCTOU atk can obtain information about memory pointed by
      // curr->next.
      Block *next = curr->next;
      while (check_block_ownership(ta_num, next) && next->free) {
        // This assumes that the blocks are contiguous. This is correct unless
        // the implementation has bugs or the heap structure is corrupted
        // (tampering).
...
void internal_TEE_MemMove(void *dest, void *src, size_t size, uint8_t ta_num) {
  if (ta_num == CORE_NUM) {
    // NOTE: unmodified from previous versions. It is not clear why the TEE
    // would use this function.
    if ((dest < TEE_CORE_MEMORY_START_ADDR) ||
        (dest + size > TEE_CORE_MEMORY_END_ADDR) ||
        (src < TEE_CORE_MEMORY_START_ADDR) ||
        (src + size > TEE_CORE_MEMORY_END_ADDR)) {
      ERR_MSG("Mem move error");
      return;
    }
  } else {
    ASSERT_OUTBUF(ta_num, dest, size) {
      ERR_MSG("Mem move error");
      return;
    }

    ASSERT_INBUF(ta_num, src, size) {
      ERR_MSG("Mem move error");
      return;
    }
  }
 ...
 * @param ta_num TA number (1 or 2)
 */
void internal_TEE_MemFill(void *buffer, uint8_t x, size_t size,
                          uint8_t ta_num) {

  if (ta_num == CORE_NUM) {
    // NOTE: unmodified from previous version.
    if ((buffer < TEE_CORE_MEMORY_START_ADDR) ||
        (buffer + size > TEE_CORE_MEMORY_END_ADDR)) {
      ERR_MSG("Mem fill function error");
      return;
    }
  } else {

    ASSERT_OUTBUF(ta_num, buffer, size) {
      ERR_MSG("Mem fill function error");
      return;
    }
  }
 ...
 *
 * @param objectType please check the global platform documentation page 141,
 * table 5.9
 * @param maxObjectSize max key size
 * @param object pointer to the object handle
 * @param ta_num TA number (1 or 2)
 *
 * @return TEE_SUCCESS if the object is allocated successfully, TEE_FAILED
 * otherwise
 */
TEE_Result internal_TEE_AllocateTransientObject(uint32_t objectType,
  ...
  ASSERT_OUT(ta_num, (void *)object, sizeof(TEE_ObjectHandle)) {
    ERR_MSG("Invalid object pointer");
    return TEE_ERROR_BAD_PARAMETERS;
  }

  __TEE_ObjectHandle *obj = allocate_object_handler(
      ta_num, 0, maxObjectSize / 8, 0, TEE_OBJ_TYPE_TRANSIENT);
  if (obj == NULL) {
 ...
 * @param ta_num TA number (1 or 2)
 *
 * @return TEE_SUCCESS if the attributes are populated successfully, TEE_FAILED
 * otherwise
 */
...
TEE_Result internal_TEE_PopulateTransientObject(TEE_ObjectHandle object,
  ...
  if (check_mem_ownership(ta_num, (void *)attrs,
                          sizeof(TEE_Attribute) * attrCount) != true) {
...
void internal_TEE_InitRefAttribute(TEE_Attribute *attr, uint32_t attributeID,
  ...
  if (check_mem_accessibility(ta_num, buffer, length) != true) {
    return;
  }
 ...
 * @param ta_num TA number
 *
 * @return TEE_SUCCESS if the attribute is found and returned, TEE_FAILED
 * otherwise
 *
 */
TEE_Result internal_TEE_GetObjectBufferAttribute(TEE_ObjectHandle object,
                                                 uint32_t attributeID,
                                                 /*[outbuf]*/ void *buffer,
                                                 size_t *size, uint8_t ta_num) {
  if (!check_object_ownership(ta_num, object)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }

  ASSERT_INOUT(ta_num, size, sizeof(*size)) { return TEE_ERROR_BAD_PARAMETERS; }

  ASSERT_OUTBUF(ta_num, buffer, *size) { return TEE_ERROR_BAD_PARAMETERS; }

  __TEE_ObjectHandle *obj = (__TEE_ObjectHandle *)object;

  for (int i = 0; i < 4; i++) {
    if (obj->attrs[i].attributeID == attributeID) {
      if (check_mem_ownership(ta_num, (void *)obj->attrs[i].content.ref.buffer,
                              obj->attrs[i].content.ref.length) != true) {
        return TEE_ERROR_BAD_PARAMETERS;
      }
      if (check_mem_ownership(ta_num, (void *)size, sizeof(size_t)) != true) {
        return TEE_ERROR_BAD_PARAMETERS;
      }
      memcpy(buffer, obj->attrs[i].content.ref.buffer,
 ...
 * otherwise
 */
TEE_Result internal_TEE_GetObjectValueAttribute(TEE_ObjectHandle object,
                                                uint32_t attributeID,
  ...
  if (check_mem_ownership(ta_num, (void *)a, sizeof(uint32_t)) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (check_mem_ownership(ta_num, (void *)b, sizeof(uint32_t)) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
...
void internal_TEE_FreeTransientObject(TEE_ObjectHandle object, uint8_t ta_num) {
  ...
  deallocate_object_handler(temp_obj);
}

/**
 * @brief Close an object (transient or persistent) by freeing the allocated
 * memory
 *
 * @param object pointer to the object handle
 */
...
TEE_Result internal_TEE_ReadObjectData(TEE_ObjectHandle object,
  ...
  if (check_mem_ownership(ta_num, (void *)count, sizeof(size_t)) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (check_mem_ownership(ta_num, (void *)buffer, size) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  ...
  *count = 0; // Readed size is 0
  if (obj->obj_storage_type ==
      TEE_OBJ_TYPE_PERSISTENT) // Check if it is a persistent object
  {
    ...
    flash_internalRead((uint8_t *)temp_buff, total_size - free_size,
                       start_addr);

    // Extract the object data from the raw data
    *count = flash_readObject((const char *)temp_buff, total_size - free_size,
                              obj->obj_id, buffer, size);
    if (*count <= 0)
      return TEE_FAILED;
  }
...
TEE_Result internal_TEE_WriteObjectData(TEE_ObjectHandle object,
  ...
  // This method can change later according to the core api documentation page
  // 173 and descprition of this function The mechanism in the documentation
  // assume that there is a file system and overwrited data will start at the
  // current position However there is not fs in this case, and for now, if the
  // buffer is already allocated, do not overwrite

  if (obj->buffer) { // Check if the buffer is already allocated (object already
                     // exists)
    return TEE_FAILED;
  } else {
    obj->buffer = (char *)internal_TEE_Malloc(size * sizeof(char),
                                              TEE_MALLOC_NO_FILL, CORE_NUM);
    if (!obj->buffer) {
 ...
 *
 * @param storageID storage identifier used in this implementation to pass the
 * TA number (as otherwise it is not used)
 ...
 * @param object pointer to the object handle
 *
 * @return TEE_SUCCESS if the object is created successfully, TEE_FAILED
 * otherwise
 */
TEE_Result internal_TEE_CreatePersistentObject(
    uint32_t storageID,
    /*[in(newObjectIDLen)]*/ void *objectID, size_t objectIDLen, uint32_t flags,
    TEE_ObjectHandle attributes,
    /*[inbuf]*/ void *initialData, size_t initialDataLen,
    /*[outopt]*/ TEE_ObjectHandle *object, uint8_t ta_num) {
  ASSERT_IN(ta_num, objectID, objectIDLen) { return TEE_ERROR_BAD_PARAMETERS; }
  ASSERT_INBUF(ta_num, initialData, initialDataLen) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  ...
  // The previous implementation writes the obj_id later.
  __TEE_ObjectHandle *temp_obj = allocate_object_handler(
      ta_num, 0, initialDataLen, flags, TEE_OBJ_TYPE_PERSISTENT);
  if (!temp_obj) {
    return TEE_FAILED;
  }
  ...
  if (((flags | TEE_OBJ_JUST_WRITE) == TEE_OBJ_JUST_WRITE) ||
      ((flags | TEE_OBJ_READ_AND_WRITE) == TEE_OBJ_READ_AND_WRITE)) {
    // Initialize the cJSON hooks (to be able to use cJSON functions and avoid
    // NULL pointer errors)
    cJSON_InitHooks(NULL);

    // NOTE: storageID was not used in the current implementation so it is used
    // to indicate the TA number NOTE 2: use int number as a string in objectID
    // parameter If you want to use strings, please use the following:
    // memcpy(&temp_obj->obj_id, objectID, objectIDLen);
    // temp_obj->obj_id = atoi(objectID);

    if (flash_writeNewObject((const char *)initialData, initialDataLen,
                             temp_obj->obj_id, storageID) < 0) {
      deallocate_object_handler(temp_obj);
 ...
 * @param object pointer to the object handle
 *
 * @return TEE_SUCCESS if the object is opened successfully, TEE_FAILED
 * otherwise
 */
TEE_Result internal_TEE_OpenPersistentObject(
    uint32_t storageID,
    /*[in(objectIDLength)]*/ void *objectID, size_t objectIDLen, uint32_t flags,
    /*[out]*/ TEE_ObjectHandle *object, uint8_t ta_num) {
  ASSERT_IN(ta_num, objectID, objectIDLen) { return TEE_ERROR_BAD_PARAMETERS; }
  ASSERT_OUT(ta_num, (void *)object, sizeof(TEE_ObjectHandle)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  ...
  // NOTE: storageID was not used in the current implementation so it is used to
  // indicate the TA number Get information about the secure storage area
  ...
  size_t size = flash_objectSize(temp_buff, total_size - free_size, obj_id);

  __TEE_ObjectHandle *temp_obj = allocate_object_handler(
      ta_num, obj_id, size, flags, TEE_OBJ_TYPE_PERSISTENT);
  if (temp_obj == NULL) {
    ERR_MSG("Failed to allocate object");
    return TEE_FAILED;
  }

  int ret = flash_readObject((const char *)temp_buff, total_size - free_size,
                             temp_obj->obj_id, temp_obj->buffer, temp_obj->len);
  if (ret <= 0) {
    deallocate_object_handler(temp_obj);
...
/**
 * @brief Close and delete a persistent object in the secure storage area of a
 * TA
 *
 * @param object pointer to the object handle
 * @param ta_num TA number
 *
 * @return TEE_SUCCESS if the object is closed and deleted successfully,
 * TEE_FAILED otherwise
 */
TEE_Result internal_TEE_CloseAndDeletePersistentObject(TEE_ObjectHandle object,
                                                       uint8_t ta_num) {
  if (!check_object_ownership(ta_num, object)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
 ...
 *
 * @return TEE_SUCCESS if the conversion is successful, TEE_ERROR_OVERFLOW
 * otherwise
 */
TEE_Result internal_TEE_BigIntConvertToS32(int32_t *dest, TEE_BigInt *src,
                                           uint8_t ta_num) {
  if (check_mem_ownership(ta_num, (void *)dest, sizeof(int32_t)) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (check_mem_ownership(ta_num, (void *)src, sizeof(TEE_BigInt)) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (*src > 0x7FFFFFFF)
 ...
 *
 * @return -1 if the big integer is less than the short value, 0 if they are
 * equal, 1 if the big integer is greater than the short value
 */
int32_t internal_TEE_BigIntCmpS32(TEE_BigInt *op, int32_t shortVal,
                                  uint8_t ta_num) {
  if (check_mem_ownership(ta_num, (void *)op, sizeof(TEE_BigInt)) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
 ...
 *
 * @param dest pointer to the destination of the operation where the result is
 * stored
...
void internal_TEE_BigIntMod(TEE_BigInt *dest, TEE_BigInt *op, TEE_BigInt *n,
                            uint8_t ta_num) {
  if (check_mem_ownership(ta_num, (void *)dest, sizeof(TEE_BigInt)) != true) {
    return;
  }
  if (check_mem_ownership(ta_num, (void *)op, sizeof(TEE_BigInt)) != true) {
    return;
  }
  if (check_mem_ownership(ta_num, (void *)n, sizeof(TEE_BigInt)) != true) {
    return;
  }
 ...
 */
void internal_TEE_BigIntDiv(TEE_BigInt *dest_q, TEE_BigInt *dest_r,
                            TEE_BigInt *op1, TEE_BigInt *op2, uint8_t ta_num) {
  if (check_mem_ownership(ta_num, (void *)dest_q, sizeof(TEE_BigInt)) != true) {
    return;
  }
  if (check_mem_ownership(ta_num, (void *)dest_r, sizeof(TEE_BigInt)) != true) {
    return;
  }
  if (check_mem_ownership(ta_num, (void *)op1, sizeof(TEE_BigInt)) != true) {
    return;
  }
  if (check_mem_ownership(ta_num, (void *)op2, sizeof(TEE_BigInt)) != true) {
    return;
  }
 ...
 * @param sign sign of the value into the buffer that will be the sign of the
 * big integer
 *
 * @return TEE_SUCCESS if the conversion is successful, TEE_ERROR_OVERFLOW
 * otherwise
 */
TEE_Result internal_TEE_BigIntConvertFromOctetString(TEE_BigInt *dest,
                                                     uint8_t *buffer,
                                                     size_t bufferLen,
                                                     int32_t sign,
                                                     uint8_t ta_num) {
  ...
  if (check_mem_ownership(ta_num, (void *)buffer, bufferLen) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
...
void internal_TEE_BigIntInit(TEE_BigInt *bigInt, size_t len, uint8_t ta_num) {
  if (check_mem_ownership(ta_num, (void *)bigInt, len) != true) {
    return;
  }
 ...
 *
 * @return If it is success, returns MBEDTLS PSA API algorithm types else
 * returns 0
 */
static uint32_t get_algorithmType(uint32_t type) {
  ...
  case TEE_ALG_ECDSA_SHA3_256:
    return PSA_ALG_ECDSA(MBEDTLS_MD_SHA3_256);
  ...
  case TEE_ALG_ECDSA_SHA3_384:
    return PSA_ALG_ECDSA(PSA_ALG_SHA3_384);
  ...
  case TEE_ALG_ECDSA_SHA3_512:
    return PSA_ALG_ECDSA(PSA_ALG_SHA3_512);
 ...
 * @param type CORE API algorithm types
 *
 * @warning This function must use for just asymmetric signature and
 * verification
 *
 * @return If it is success, returns MBEDTLS PSA API algorithm types else
 * returns 0
 ...
 * @param key pointer to the object handle, were attributes are stored and were
 * the key will be stored
 *
 * @return TEE_SUCCESS if the key pair is generated successfully, TEE_FAILED
 * otherwise
 */
static int generate_ecdsaKeyPair(__TEE_OperationHandle *op,
                                 __TEE_ObjectHandle *key) {
  // Initialize neccessary things for set x and y coordinate and generate key
  // pair for eliptic curve
  ...
  // The attributes contains the value of X and Y coordinates of the public key
  // used for key generation of the private key
  int ret = mbedtls_ctr_drbg_seed(&op->ctr_drbg, mbedtls_entropy_func,
                                  &op->entropy, NULL, 0);
  if (ret != 0) {
  ...
  ret = mbedtls_mpi_read_binary(
      &op->ctx_sign.private_d,
      (const unsigned char *)key->attrs[0].content.ref.buffer,
  ...
  ret = mbedtls_mpi_read_binary(
      &op->ctx_sign.private_Q.private_X,
      (const unsigned char *)key->attrs[1].content.ref.buffer,
  ...
  ret = mbedtls_mpi_read_binary(
      &op->ctx_sign.private_Q.private_Y,
      (const unsigned char *)key->attrs[2].content.ref.buffer,
      key->attrs[2].content.ref.length);
  if (ret != 0) {
    ERR_MSG("mbedtls_mpi_read_binary failed for private key\n");
    goto end;
  }
  ret = mbedtls_ecdsa_genkey(&op->ctx_sign, alg, mbedtls_ctr_drbg_random,
                             &op->ctr_drbg);
  if (ret != 0) {
...
static uint32_t get_modeType(uint32_t mode) {
  ...
  case TEE_MODE_SIGN:
    return PSA_KEY_USAGE_SIGN_HASH | PSA_KEY_USAGE_VERIFY_HASH;
  case TEE_MODE_VERIFY:
    return PSA_KEY_USAGE_VERIFY_HASH | PSA_KEY_USAGE_SIGN_HASH;
  case TEE_MODE_MAC:
 ...
 *
 * @return If it is success, MBEDTLS PSA API key type else
...
static uint32_t convert_alg2KeyType(uint32_t alg) {
  ...
  default:
    return PSA_KEY_USAGE_VERIFY_MESSAGE;
  }
}
...
static uint32_t get_keyType(uint32_t mode, uint32_t alg) {
  ...
  case TEE_MODE_SIGN:
    return convert_alg2KeyType(alg);
  case TEE_MODE_VERIFY:
    return PSA_KEY_USAGE_VERIFY_MESSAGE;
  case TEE_MODE_MAC:
...
/**
 * @brief Returns key len in bytes according to the given TEE CORE API crpyto
 * algorithm type
 *
...
/**
 * @brief Check if the expected key len in bytes, according to the given TEE
 * CORE API crypto algorithm type, matches the given key len
 *
 * @param alg TEE Core API crypto algorithm
...
/**
 * @brief Allocates the psa cipher operation struct (of MBEDTLS) and sets the
 * default init settings
 *
 * @return In case of success, it returns psa_cipher_operation_t struct else
 * NULL
 */
static psa_cipher_operation_t *cipher_operationInit(void) {
  psa_cipher_operation_t *cipher_op =
      (psa_cipher_operation_t *)internal_TEE_Malloc(
          sizeof(psa_cipher_operation_t), TEE_MALLOC_FILL_ZERO, CORE_NUM);
  if (!cipher_op) {
    return NULL;
  }
...
/**
 * @brief This function release the memory allocated by psa_cipher_operation_t
 * struct
 */
...
/**
 * @brief Allocates the psa hash operation struct and sets the default init
 * settings
 *
...
static psa_hash_operation_t *hash_operationInit(uint32_t alg) {
  psa_hash_operation_t *hash_op = (psa_hash_operation_t *)internal_TEE_Malloc(
      sizeof(psa_hash_operation_t), TEE_MALLOC_FILL_ZERO, CORE_NUM);
  if (!hash_op)
    return NULL;
...
/**
 * @brief This function release the memory allocated for the
 * psa_hash_operation_t struct
 */
...
/**
 * @brief Allocates the psa mac operation struct and sets the default init
 * settings
 *
 * @return If it's succes, it returns psa_mac_operation_t struct else NULL
 */
static psa_mac_operation_t *mac_operationInit(void) {
  psa_mac_operation_t *mac_op = (psa_mac_operation_t *)internal_TEE_Malloc(
      sizeof(psa_mac_operation_t), TEE_MALLOC_FILL_ZERO, CORE_NUM);
  if (!mac_op)
    return NULL;
...
/**
 * @brief This function release the memory allocated for the psa_mac_operation_t
 * struct
 */
...
/**
 * @brief Initializes the correct PSA API CRYPTO structures based on the given
 * mode and algorithm
 *
 * @param op    TEE operation Handle that holds the relevant parameter for
 * crypto opeartions
 * @param mode  Crypto operations type (e.g., Encrpyt, Decrpyt, Sign, Verify
 * etc.)
 * @param alg   Crypto operation algorithm
 *
 * @return 0 if the operation is initialized successfully, -1 otherwise
 */
static int mbed_operationsInit(__TEE_OperationHandle *op, uint32_t mode,
                               uint32_t alg) {
  switch (mode) {
  ...
  case TEE_MODE_DIGEST:
    // If there is no key set for signinf and verification of the proccess, we
    // need to initalize PSA API
...
/**
 * @brief Deinitialize PSA API Crpyto operations based on the given mode and
 * algorithm
 *
 * @param op    TEE operation Handle that holds the relevant parameters for
 * crypto opeartions
 * @param mode  Crypto operations type (e.g., Encrpyt, Decrpyt, Sign, Verify
 * etc.)
 *
...
static void mbed_operationsDeInit(__TEE_OperationHandle *op, uint32_t mode) {
  ...
  case TEE_MODE_ENCRYPT:
  case TEE_MODE_DECRYPT:
    cipher_operationDeinit(op->cipher_op);
    break;
  //   Asymmetric Functios
  /*case TEE_MODE_SIGN:
  ...
  //   MAC Functions
  case TEE_MODE_MAC:
    mac_operationDeInit(op->mac_op);
    break;
  //  Message Digest(HASH) Functions
  case TEE_MODE_DIGEST:
    hash_operationDeInit(op->hash_op);
    break;
  default:
    ERR_MSG("Invalid operation mod");
    return;
...
/**
 * @brief Allocates a new operation handle for the given algorithm, mode, and
 * maximum key size
 *
 ...
 *
 * @return TEE_SUCCESS if the operation is allocated successfully, TEE_FAILED
 * otherwise
 */
TEE_Result internal_TEE_AllocateOperation(TEE_OperationHandle *operation,
                                          uint32_t algorithm, uint32_t mode,
                                          uint32_t maxKeySize, uint8_t ta_num) {
  __TEE_OperationHandle *temp_op = (__TEE_OperationHandle *)internal_TEE_Malloc(
      sizeof(__TEE_OperationHandle), TEE_MALLOC_NO_FILL, CORE_NUM);
  if (!temp_op) {
  ...
  // Initialize relavent mode for psa crytpo api (like encryption, signature,
  // mac etc.)
...
void internal_TEE_FreeOperation(TEE_OperationHandle operation, uint8_t ta_num) {
  ...
  // if you want to set x and y values for eliptic curve for signature and
  // verification. The if statement is used to check if the key_id is set or
  // not, which allows us to understand which PSA API was used for the operation
  // and which elements should be freed.
...
TEE_Result internal_TEE_SetOperationKey(TEE_OperationHandle operation,
                                        TEE_ObjectHandle key, uint8_t ta_num) {
  if (!check_operation_ownership(ta_num, operation)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  ...
  // Extract the attributes from the object to understand if the key should be
  // generated or set
  int attr_flag = (obj->attrs[0].attributeID + obj->attrs[1].attributeID +
                   obj->attrs[2].attributeID + obj->attrs[3].attributeID) > 0
                      ? 1
                      : 0;
  if ((obj->obj_type == TEE_TYPE_ECDSA_KEYPAIR) && (attr_flag != 0)) {
    // If the key is ECDSA key pair and the attributes are set, then the key
    // should be generated starting from the attributes that contain the x and y
    // coordinates of the public key
    op->key_id = 0;
    return generate_ecdsaKeyPair(op, obj);
  }

  // Check if the key size (stored in the obj or generated before) is correct
  ...
  int res = psa_import_key(&attributes, (const uint8_t *)obj->buffer,
                           obj->key_len / 8, &op->key_id);
  if (res != PSA_SUCCESS) {
...
/**
 * @brief Generate a key (based on the parameters and key size passed) and
 * stored it inside the object (persistently or not based on the object type)
 *
 ...
 * @param ta_num number of the trusted application that identify which secure
 * storage area the object will be stored in (if it is persistent)
 *
 * @return TEE_SUCCESS if the key is generated successfully, TEE_FAILED
 * otherwise
 */

TEE_Result internal_TEE_GenerateKey(TEE_ObjectHandle object, uint32_t keySize,
  ...
  // Generate Key
  if (mbedtls_ctr_drbg_random(&ctr_drbg, (unsigned char *)temp_buffer,
                              keySize / 8) != 0) {
  ...
  if (obj->obj_storage_type != TEE_OBJ_TYPE_PERSISTENT) {
    ...
    if (internal_TEE_WriteObjectData(object, temp_buffer, keySize / 8,
                                     ta_num) != TEE_SUCCESS) {
...
/**
 * @brief Generate a random number (using hardware RNG) and store it in the
 * buffer
 *
 * @param randomBuffer pointer to the buffer where the random number will be
 * stored
...
void internal_TEE_GenerateRandom(void *randomBuffer, size_t randomBufferLen,
                                 uint8_t ta_num) {
  if (check_mem_ownership(ta_num, randomBuffer, randomBufferLen) != true) {
    return;
  }
  uint32_t rnum = 0;
  ...
  do {
    ...
    // Remove if there is a number that bigger than 32 because it cause a
    // problem for cjson. To do that just need to set the first msb zero,
    // 0b01111111
    rnum = rnum & 0x7F7F7F7F;
    memcpy(val, &rnum, 4);

    // Remove if there is a number that smaller than 32 because it cause a
    // problem for cjson
    ...
    len = len + 4; // even if offset values is smaller than 4, increase 4 to
                   // break the loop
...
/**
 * @brief Initializes the cipher operation for symmetric encryption/decryption
 * given the initialization vector (IV) Supported algorithms: AES ECB, AES CBC,
 * AES CTR, AES XTS, AES CBC MAC, AES CMAC, AES CCM, AES GCM
 *
 * @param operation pointer to the operation handle
 * @param IV pointer to the initialization vector
 * @param IVLen length of the initialization vector
 */
void internal_TEE_CipherInit(TEE_OperationHandle operation, void *IV,
                             size_t IVLen, uint8_t ta_num) {
  if (!check_operation_ownership(ta_num, operation)) {
    return;
  }
  ...
  if (check_mem_ownership(ta_num, IV, IVLen) != true) {
    return;
  }
  ...
  if (op->info.mode == TEE_MODE_ENCRYPT) {
    if (psa_cipher_encrypt_setup((psa_cipher_operation_t *)op->cipher_op,
                                 op->key_id, algorithm) != PSA_SUCCESS) {
      ERR_MSG("Failed to setup cipher encrypt");
      return;
    }
  } else if (op->info.mode == TEE_MODE_DECRYPT) {
    if (psa_cipher_decrypt_setup((psa_cipher_operation_t *)op->cipher_op,
                                 op->key_id, algorithm) != PSA_SUCCESS) {
  ...
  if (psa_cipher_set_iv((psa_cipher_operation_t *)op->cipher_op,
                        (const uint8_t *)IV, IVLen) != PSA_SUCCESS) {
...
/**
 * @brief Updates the cipher operation by computing the cipher of the input data
 * and storing the result in the output buffer
 *
 * @param operation pointer to the operation handle (with the key, IV and PSA
 * cipher operation set)
 ...
 *
 * @return TEE_SUCCESS if the cipher operation is updated successfully,
 * TEE_FAILED otherwise
 */
TEE_Result internal_TEE_CipherUpdate(TEE_OperationHandle operation,
                                     /*[inbuf]*/ void *srcData, size_t srcLen,
                                     /*[outbuf]*/ void *destData,
                                     size_t *destLen, uint8_t ta_num) {
  if (!check_operation_ownership(ta_num, operation)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (check_mem_ownership(ta_num, srcData, srcLen) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (check_mem_ownership(ta_num, (void *)destLen, sizeof(destLen)) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (check_mem_ownership(ta_num, destData, *destLen) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  ...
  // destLen now rapresent the logical size of the output buffer as a return
  // parameter destSize is the physical size of the output buffer Both len and
  // size are passed as a parameter to the PSA API
  size_t destSize = *destLen;
  *destLen = 0;

  // Compute the actual cipher of the input data and store the result in the
  // output buffer
  if (psa_cipher_update((psa_cipher_operation_t *)op->cipher_op,
                        (const uint8_t *)srcData, srcLen, destData, destSize,
                        destLen) != PSA_SUCCESS) {
...
/**
 * @brief Finalizes the cipher operation by computing the final cipher of the
 * input data (if other data are passed) and storing the result in the output
 * buffer
 *
 * @param operation pointer to the operation handle (with the key, IV and PSA
 * cipher operation set)
 ...
 * @param destLen length of the output data
 *
 * @return TEE_SUCCESS if the cipher operation is finalized successfully,
 * TEE_FAILED otherwise
 */
TEE_Result internal_TEE_CipherDoFinal(TEE_OperationHandle operation,
  ...
  if (check_mem_ownership(ta_num, (void *)destLen, sizeof(size_t)) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (check_mem_ownership(ta_num, destData, *destLen) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  ...
  // destLen now rapresent the logical size of the output buffer as a return
  // parameter destSize is the physical size of the output buffer
  size_t remaining_size = *destLen;
  size_t produced_size = 0;
  *destLen = 0;

  // If the input data is not empty, update the cipher operation with the input
  // data
  if (srcData && srcLen > 0) {

    if (psa_cipher_update((psa_cipher_operation_t *)op->cipher_op,
                          (const uint8_t *)srcData, srcLen, destData,
                          remaining_size, &produced_size) != PSA_SUCCESS) {
      ERR_MSG("Failed to update cipher");
      psa_cipher_abort((psa_cipher_operation_t *)op->cipher_op);
      return TEE_FAILED;
    }
  ...
  *destLen += produced_size;
  ...
  // Finalize the cipher operation and store the final cipher in the output
  // buffer using the PSA API
  if (psa_cipher_finish((psa_cipher_operation_t *)op->cipher_op,
                        destData + produced_size, remaining_size,
                        &produced_size) != PSA_SUCCESS) {
...
/**
 * @brief Setup the the MAC (Message Authentication Code) operation given the
 * initialization vector (IV) Supported algorithms: HMAC SHA256, HMAC SHA384,
 * HMAC SHA512
 *
 * @param operation pointer to the operation handle
 * @param IV pointer to the initialization vector
 ...
 */
void internal_TEE_MACInit(TEE_OperationHandle operation, void *IV, size_t IVLen,
                          uint8_t ta_num) {
  if (!check_operation_ownership(ta_num, operation)) {
    return;
  }
...
/**
 * @brief Updates the MAC operation by computing the MAC of the check given as
 * input
 *
 * @param operation pointer to the operation handle (with the key and PSA MAC
 * operation set)
...
/**
 * @brief Finalizes the MAC operation by computing the final MAC of the input
 * data (if needed) and storing the result in the output buffer
 *
 * @param operation pointer to the operation handle (with the key and PSA MAC
 * operation set)
 ...
 *
 * @return TEE_SUCCESS if the MAC operation is finalized successfully,
 * TEE_FAILED otherwise
 */
TEE_Result internal_TEE_MACComputeFinal(TEE_OperationHandle operation,
  ...
  // We allow NULL messages to just finalize the MAC computation.
  if (check_mem_accessibility(ta_num, message, messageLen) != true) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (!check_mem_ownership(ta_num, (void *)macLen, sizeof(size_t))) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (!check_mem_accessibility(ta_num, mac, *macLen)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  __TEE_OperationHandle *op = (__TEE_OperationHandle *)operation;

  // If the input data is not empty, update the MAC operation with the input
  // data
  ...
  // Swap the physical size of the output buffer with the logical size of the
  // output buffer
  size_t macSize = *macLen;
  *macLen = 0;

  // Finalize the MAC operation and store the final MAC in the output buffer
  // using the PSA API
...
/**
 * @brief Finalizes the hash operation by computing the final hash of the input
 * data (if needed) and storing the result in the output buffer
 *
 * @param operation pointer to the operation handle (with the PSA hash operation
 * set)
 * @param chunk pointer to the input data
...
TEE_Result internal_TEE_DigestDoFinal(TEE_OperationHandle operation,
  ...
  // In order to call TEE_DigestDoFinal function multiple time by sequentially,
  // the process should not be ended After calling psa_hash_finish function,
  // operation id param is set to 0 and it prevents to use other hash functions
  // Hence, we need to clone the hash operation and finalize it or re-call
  ...
  // If the input data is not empty, update the hash operation with the input
  // data
  if (chunk && chunkLen > 0) {
    psa_status_t err =
        psa_hash_update(&target, (const uint8_t *)chunk, chunkLen);
    if (err != 0) {
  ...
  *hashLen = 0;
  ...
  // Finalize the hash operation and store the final hash in the output buffer
  // using the PSA API
...
/**
 * @brief Extract the hash value from the hash operation and store it in the
 * output buffer
 *
 * @param operation pointer to the operation handle (with the PSA hash operation
 * set)
...
TEE_Result internal_TEE_DigestExtract(TEE_OperationHandle operation,
  ...
  // PSA HASH doesn't support extraction of hash value which is still going on,
  // for this reason we clone the operation and finalized it to extract the
  // digests Using this trick we will not affect the original hash operation
  psa_hash_operation_t target = PSA_HASH_OPERATION_INIT;
  if (psa_hash_clone(op->hash_op, &target) != 0) {
  ...
  *hashLen = 0;
  ...
  // Finalize the clone of the hash operation and to be able to store it in the
  // output buffe
  if (psa_hash_finish(&target, hash, hashSize, hashLen) != 0) {
    ERR_MSG("Digest extraction failed");
    psa_hash_abort(&target);
...
/**
 * @brief Initializes the asymmetric signature operation for signing a message
 * given its hash
 *
 * @param operation pointer to the operation handle
 ...
 *
 */
TEE_Result internal_TEE_AsymmetricSignDigest(
    TEE_OperationHandle operation,
    /*[in]*/ TEE_Attribute *params, uint32_t paramCount,
    /*[inbuf]*/ void *digest, size_t digestLen,
    /*[outbuf]*/ void *signature, size_t *signatureLen, uint8_t ta_num) {
  if (!check_operation_ownership(ta_num, operation)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (!check_mem_ownership(ta_num, (void *)params,
                           sizeof(TEE_Attribute) * paramCount)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (!check_mem_accessibility(ta_num, digest, digestLen)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (!check_mem_ownership(ta_num, (void *)signatureLen, sizeof(size_t))) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (!check_mem_accessibility(ta_num, signature, *signatureLen)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  ...
  uint32_t alg = 0;
  // If the key is set, use the PSA API with internal generation of X and Y
  // params for signing the hash (key_id is set in the operation in this case)
  // Otherwise, use the MbedTLS API with custom X and Y params for signing the
  // hash
  if (op->key_id != 0) {
    alg = get_algorithmType(op->info.algorithm);
    // Use the PSA API to sign the hash
    psa_status_t status =
        psa_sign_hash(op->key_id, alg, (const uint8_t *)digest, digestLen,
    ...
    int ret = mbedtls_ecdsa_write_signature(
        &op->ctx_sign, alg, digest, digestLen, signature, *signatureLen,
        signatureLen, mbedtls_ctr_drbg_random, &op->ctr_drbg);
    if (ret != 0) {
...
/**
 * @brief Initializes the asymmetric verification operation for verifying the
 * signatura of given message hash
 *
 * @param operation pointer to the operation handle
 ...
 *
 * @return TEE_SUCCESS if the signature is verified successfully, TEE_FAILED
 * otherwise (invalid signature or errors)
 */
TEE_Result internal_TEE_AsymmetricVerifyDigest(
    TEE_OperationHandle operation,
    /*[in] */ TEE_Attribute *params, uint32_t paramCount,
    /*[inbuf]*/ void *digest, size_t digestLen,
    /*[inbuf]*/ void *signature, size_t signatureLen, uint8_t ta_num) {
  if (!check_operation_ownership(ta_num, operation)) {
    return TEE_ERROR_BAD_PARAMETERS;
  }
  if (!check_mem_ownership(ta_num, (void *)params,
  ...
  if (op->key_id != 0) {
    ...
    uint32_t alg = get_algorithmType(op->info.algorithm);
    psa_status_t status =
        psa_verify_hash(op->key_id, alg, (const uint8_t *)digest, digestLen,
    ...
    int ret = mbedtls_ecdsa_read_signature(&op->ctx_verify, digest, digestLen,
