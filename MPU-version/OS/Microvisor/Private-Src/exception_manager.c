...
int Get_Exception_Priority(int exception_number) {
  ...
  if (4 <= exception_number && exception_number <= 15) {
    ...
    /* External interrupts */
    /* The mapping between the extern interrup number and the exception number
     * is shifted by 16 */
    pri_bits =
        NVIC->IP[exception_number - 16]; // external interrupt 0 = exception 16
  }
 ...
 * BASEPRI: if the exception pre-empted another deprioritized interrupt
 * execution, than it's necessary to save its priority in order to restore it
 * later (once this new deprioritized interrupt has been handled) otherwise the
 * restored code would execute with the wrong priority! Note that the priority
 * of the pre-empted deprioritized interrupt will be certainly stored in BASEPRI
 * because it must be lower than 0 (since the currently running code cannot have
 * priority higher than 0 and was able to successfully pre-empt the previously
 * executing one) If BASEPRI has value 0, it means that "regular" code running
 * at the lowest possible priority (since BASEPRI set to 0 does not mask
 * priority) was pre-empted.
 *
 * EXCRETURN: this value must be stored in order to set the correct stack and
 * mode when returning form the Exception_Simulator routine.
 *
 */
__attribute__((naked)) void Exception_Catcher() {
  __asm__(
      /* Disable interrupts, note that FAULTMASK will be cleared on return from
         exception */
      "cpsid if\n"
      ...
      /* Set CTRL.nPRIV to 0 (must allow Execution Simulator to act in
         privileged mode) */
      "mrs r0, CONTROL\n"
      "and r0, #0\n"
      "msr CONTROL, r0\n"
      ...
      "ldr r6, =Exception_Simulator\n" // Set the address of Exception_Simulator
                                       // function as RetAddr
      ...
      "push {r3}\n"
      "bl Get_Exception_Priority\n" // get priority of the exception (return
                                    // value saved in r0)
      "pop {r3}\n"
      "mov r1, r0\n"   // execution priority
      "mrs r0, IPSR\n" // set exception to execute
 ...
 *
 * After the execution of the ISR the function should re-gain privileges and
 * setup for the return to the pre-empted execution, unfortunatly this cannot be
 * done straightforward for two reasons:
 * 1. CONTROL.nPRIV is set to 1 (no privileges) and cannot be changed from
 * Thread mode.
 * 2. Writes to EPSR are ignored, meaning that the only way to restore the
 * context is with a proper exception return.
 *
...
Exception_Simulator(int exception_number, int exception_priority) {
  __asm__(
      /* Set virtual IPSR value */
      "ldr r2, =_virtual_IPSR\n"
      "str r0, [r2]\n" // Store in the virtual_IRSR the exception number (passed
                       // as parameter in r0)
      ...
      "cmp r1, #0\n"
      "beq .PRI_SET\n" // requested priority 0, already set (branch to .PRI_SET
                       // label)
      ...
      /* Since we are executing unprivileged code, an access to the PPB will
         trigger a HardFault */
 ...
 * Only done when the HardFault originates form a pre-determined instruction
 * range (within the Exception_Simulator function).
 *
 * Parameters:
...
__attribute__((naked)) void Exception_Return_Handler(unsigned int *auto_frame) {
  __asm__(
      ...
      /**
       * Check if a exception return was requested after the execution of a
       * deprioritized handler This is done by comparing the address that caused
      ...
      "blo .NO_EXC_RET_REQUESTED\n" // PC is less than EXC_RET_START, no
                                    // exception return was requested
      "ldr r3, =EXC_RET_END\n"
      "cmp r2, r3\n"
      "bhi .NO_EXC_RET_REQUESTED\n" // PC is greater than EXC_RET_END, no
                                    // exception return was requested
      ...
      /* Clear CFSR (Configurable Fault Status Register) as the value was
         changed by the HardFault triggered on purpose */
      "ldr r2, =0xE000ED28\n"
      "ldr r3, =0xFFFFFFFF\n" // clear all bits
      "str r3, [r2]\n"
      ...
      /**
       *  No exeption return requested, check if a return from a Global Platform
       * API was requested instead This is done by comparing the address that
      ...
      "cmp r2, r3\n"
      "blo .NO_API_RET_REQUESTED\n" // PC is less than API_RET_START, no return
                                    // from api call was requested (return)
      "ldr r3, =API_RET_END\n"
      "cmp r2, r3\n"
      "bhi .NO_API_RET_REQUESTED\n" // PC is greater than API_RET_END, no return
                                    // from api call was requested (return)
      ...
      "add sp, #4\n" // remove pushed r4 (during API call) from MSP as we don't
                     // want to restore it (return value).
      ...
      "cpsie i\n"
      /**
       * Reconfigure back the MPU to the original configuration used before the
       * API call This enforce isolation and separation between TA and client
       * applications
       */
      "blx Configure_MPU\n"
      /* Write EXC_RETURN value to LR to perform the return from the exception
