 ...
 */
__attribute__((naked)) void ota_boot_reflasher(uint32_t *src, uint32_t *dst,
                                               uint32_t len) {
  __asm__(
...
uint32_t ota_erase_flash() {
  ...
  uint32_t base_page =
      (((uint32_t)&__flash_start__ - (uint32_t)&__flash_boot_start__) /
       FLASH_PAGE_SIZE); // first page dedicated to user app
  uint32_t bank_1_available =
      (FLASH_BANK_SIZE / FLASH_PAGE_SIZE) -
      base_page; // pages in bank 1 available for user app
  ...
  FLASH_EraseInitTypeDef erase_config_bank_2 = {
      FLASH_TYPEERASE_PAGES, 2, (FLASH_BANK_SIZE / FLASH_PAGE_SIZE),
      (FLASH_BANK_SIZE / FLASH_PAGE_SIZE)};
  uint32_t page_error;
...
uint32_t ota_compute_firmware_size() {
  uint32_t *last_word =
      (uint32_t *)((uint32_t)&__flash_start__ + (uint32_t)&__flash_size__);
  ...
  uint32_t size = (uint32_t)last_word - (uint32_t)&__flash_start__ +
                  4; // + 4 to include the last word
  return size;
}
...
int ota_compute_firmware_sha256(unsigned char *output) {
  // The firmware size is stored in the permanent storage (it is calculated
  // during the boot)
  if (mbedtls_sha256((unsigned char *)&__flash_start__,
                     (size_t)PERMA_STORAGE->firmware_size, output, 0) == 0) {
    return OTA_OK;
  } else {
    return OTA_ERROR;
  }
...
/**
 * \brief	Retrive the reflasher utility and updated version of the
 * microvisor from the server, load the reflasher in memory and lauch it
 */
uint32_t ota_update_microvisor(sec_comm_context *ctx) {
  ...
  /*
          ...
          The reflasher is not stored in the flash as it would consume precious
     storage space
  */
  ...
  if (sec_comm_recv(ctx, (uint8_t *)&reflasher_size, 4) !=
      4) { // read reflasher utility size
    return OTA_COMMUNICATION_ERROR;
  }
  ...
  while (remaining_size > 0) {
    ...
    if (sec_comm_recv(ctx, (uint8_t *)&packet_size, 4) !=
        4) { // read packet_size
      HAL_FLASH_Lock();
      return OTA_COMMUNICATION_ERROR;
    }
    if (packet_size >
        UPDATE_BUFFER_SIZE) { // packet_size must be lower than buffer size
      HAL_FLASH_Lock();
      return OTA_PACKET_TOO_LARGE;
    }
    if (sec_comm_recv(ctx, buffer, packet_size) !=
        packet_size) { // read whole packet
      HAL_FLASH_Lock();
      return OTA_COMMUNICATION_ERROR;
    }

    /* Reset unused buffer part (to prevent unwanted data writes in case of
     * unaligned accesses) */
    ...
    for (uint32_t packet_index = 0; packet_index < packet_size;
         packet_index += 8) {
      uint64_t d_word =
          *(uint64_t *)&buffer[packet_index]; // extract double word
      if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, current_address,
                            d_word) != HAL_OK) {
        HAL_FLASH_Lock();
  ...
  current_address = (current_address & ~(0x7)) +
                    0x8; // align current_address to next double word
  ...
  if (sec_comm_recv(ctx, (uint8_t *)&microvisor_size, 4) !=
      4) { // read microvisor size
    return OTA_COMMUNICATION_ERROR;
  }
  ...
  /* Launch reflasher */
  ota_boot_reflasher((uint32_t *)microvisor_begin,
                     (uint32_t *)&__flash_boot_start__, microvisor_size);
...
int ota_report_error(sec_comm_context *ctx) {
  ...
  size_t total_data_len =
      4 +
      (error_data_len * 4); // error length (in bytes) of error id + error data
  if (sec_comm_send(ctx, (unsigned char *)&(PERMA_STORAGE->error_id),
                    total_data_len) != total_data_len) {
    return OTA_COMMUNICATION_ERROR;
  }
  /* Read reply */
  int reply;
  if (sec_comm_recv(ctx, (unsigned char *)&reply, 4) !=
      4) { // read 4 bytes message code
    return OTA_COMMUNICATION_ERROR;
  }
...
/**
 * \brief	Check the availability of a new firmware, usng SHA, and perform
 * the update when needed. The new firmware is downloaded from the server
 */
int ota_update_firmware(sec_comm_context *ctx) {
  ...
  if (sec_comm_recv(ctx, (uint8_t *)&firmware_size, 4) !=
      4) { // read firmware size
    return OTA_COMMUNICATION_ERROR;
  }
  ...
  if (sec_comm_send(ctx, (uint8_t *)&msg_id, 4) !=
      4) { // send message_id (4 bytes)
    HAL_FLASH_Lock();
    return OTA_COMMUNICATION_ERROR;
  }
