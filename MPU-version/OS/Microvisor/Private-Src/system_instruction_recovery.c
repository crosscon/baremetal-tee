...
/**
 * Simulates the execution of CPS (system instruction) using the original
 * context stored in the auto_frame and manual_frame CPS is a 16 bit wide
 * instruction, so it is necessary to set the length to 2
 *
 * Parameters:
 ...
 *
 */
static void Simulate_CPS(unsigned int *auto_frame, unsigned int *manual_frame,
                         int32_t inst) {
...
/**
 * Simulates the execution of MRS (system instruction) using the original
 * context stored in the auto_frame and manual_frame MRS is a 32 bit wide
 * instruction, so it is necessary to set the length to 4
 *
 * Parameters:
 * faulty_inst: instruction to simulate (MRS in this case) with the necessary
 * parameters (e.g. destination register) auto_frame: pointer to frame created
 * automatically during exception entry manual_frame: pointer to frame created
 * manually before calling this function
 */
static void Simulate_MRS(unsigned int *auto_frame, unsigned int *manual_frame,
                         uint32_t inst) {
  /*
  ...
  if (read_IPSR) {
    ...
    state anyway */
    unsigned int rd_value = Get_Register_Value(
        rd, auto_frame, manual_frame); // read destination register
    unsigned int virtual_IPSR =
        Get_Virtual_IPSR(); // get current Virtual IPSR value
    rd_value = (rd_value & (~SYS_M_IPSR_MASK)) |
               virtual_IPSR; // compute new destination register value
    Set_Register_Value(rd, auto_frame, manual_frame,
                       rd_value); // update destination register value
  }
 ...
 *
 * Parameters:
 * faulty_inst: instruction to simulate (MSR in this case) with the necessary
 * parameters auto_frame: pointer to frame created automatically during
 * exception entry manual_frame: pointer to frame created manually before
 * calling this function
 */
static void Simulate_MSR(unsigned int *auto_frame, unsigned int *manual_frame,
                         uint32_t inst) {
  /*
   * Extract parameters from the MRS instruction:
   * SYSm: system control space (SCS) register
  ...
  /* Prevent the MSR instruction to override the fileds that control the stack
   * pointer and privilege level */
  if ((SYSm ^ MSR_SYS_M_CTRL_PATTERN) == 0) {
    /* MSR instruction attempting to write CONTROL register, must prevent
     * changes to nPRIV */
    ...
    __asm__ volatile("MRS %0, control"
                     : "=r"(curr_CONTROL)); // read current CONTROL register
    ...
    if (Is_Deprioritized_Handler_Active()) {
      ...
      rn_value =
          (rn_value & ~0b10) | curr_SPSEL; // overwrite SPSEL target value
    }

    Set_Register_Value(rn, auto_frame, manual_frame,
...
/**
 * Checks whether the SVC was executed in order to simulate the usage of a
 * system instruction (CPS, MRS, MSR) with privileges. If it was, the
 * instruction is simulated while making sure that CONTROL.nPRIV is not altered
 * (unprivileged code should never be able to alter it).
 *
 * Parameters:
 *  auto_frame:
 *      pointer to frame created automatically during exeption entry
 *      contains R0,R1,R2,R3,R12,LR,PC,XPSR, can be stored on Main Stack or
 * Process Stack depending on what was used before exception entry.
 *  manual_frame:
 *      pointer to frame created manually before calling this function, contains
 * the rest of the context not saved automatically by exception entry (R4-R11
 * and EXC_RETURN (LR)) is always stored on the Main Stack.
 *
 * Returns:
 * SYS_INST_OK: if the instruction was successfully simulated
 * SYS_INST_FAIL: if the instruction could not be simulated
 * SYS_INST_NOREQ: if no system instruction simulation was requested (normal
 * instruction execution)
 */
int Recover_System_Instruction(unsigned int *auto_frame,
