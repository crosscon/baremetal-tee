...
#define PACKET_LEN_BYTES                                                       \
  4 // bytes indicating packet size (4 = 32 bit unsigned integer)

RNG_HandleTypeDef sec_comm_hrng;

/**
 * @brief  Private function, decrypts new incoming packet and stores plaintext
 * into context buffer
 * @param  ctx : sec_comm_context where the plaintext will be stored
 * @retval Operation status. Return SEC_COMM_OK when operations are performed
 * successfully, SEC_COMM_ERROR otherwise
 */
 ...
 * @retval Operation status SEC_COMM_OK when operation performed successfully,
 * SEC_COMM_ERROR otherwise
 */
int32_t sec_comm_rng_generate(sec_comm_context *ctx, uint8_t *buff,
                              uint32_t len);
...
int32_t sec_comm_init(sec_comm_context *ctx, uint32_t socket) {
  ...
  mbedtls_chachapoly_init(
      &(ctx->ccp_ctx)); // init encryption/decryption context
...
int32_t sec_comm_set_key(sec_comm_context *ctx, const uint8_t *key) {
  if (0 != mbedtls_chachapoly_setkey(&(ctx->ccp_ctx),
                                     key)) // set encryption/decryption key
    return SEC_COMM_ERROR; // encryption/decryption context initialization
                           // failed
  return SEC_COMM_OK;
}

int32_t sec_comm_connect(sec_comm_context *ctx, uint8_t *ipaddr, uint16_t port,
                         uint16_t local_port) {
  if (WIFI_STATUS_OK != wifi_wrapper_open_client_connection(ctx->socket, ipaddr,
                                                            port, local_port))
...
int32_t sec_comm_send(sec_comm_context *ctx, const uint8_t *buff,
                      uint32_t len) {
  ...
  while (remaining > 0) {
    uint32_t chunk_size = remaining < BUFFER_SIZE
                              ? remaining
                              : BUFFER_SIZE; // plaintext processed in chunks of
                                             // at most BUFFER_SIZE len
    ...
    if (0 != mbedtls_chachapoly_encrypt_and_tag(
                 &ctx->ccp_ctx, chunk_size, nonce_buff, nonce_buff, NONCE_LEN,
                 &buff[len - remaining], ciphertext_buff, auth_tag_buff))
      return SEC_COMM_ERROR; // encryption failed

    /* Send packet length on socket */
    if (WIFI_STATUS_OK !=
        wifi_wrapper_send_data(ctx->socket, (uint8_t *)&chunk_size, 4))
      return SEC_COMM_ERROR; // data send failed

    /* Send ciphertext */
    if (WIFI_STATUS_OK != wifi_wrapper_send_data(ctx->socket, ciphertext_buff,
                                                 (uint16_t)chunk_size))
      return SEC_COMM_ERROR; // data send failed

    /* Send auth tag */
    if (WIFI_STATUS_OK !=
        wifi_wrapper_send_data(ctx->socket, auth_tag_buff, AUTH_TAG_LEN))
      return SEC_COMM_ERROR; // data send failed

    /* Send nonce */
    if (WIFI_STATUS_OK !=
        wifi_wrapper_send_data(ctx->socket, nonce_buff, NONCE_LEN))
      return SEC_COMM_ERROR; // data send failed
...
int32_t sec_comm_recv(sec_comm_context *ctx, uint8_t *buff, uint32_t len) {
  ...
  while (remaining > 0) {
    if (ctx->plaintext_next <
        ctx->plaintext_len) { // data available from plaintext buffer
...
int32_t sec_comm_refill_plaintext(sec_comm_context *ctx) {
  ...
  if (WIFI_STATUS_OK != wifi_wrapper_receive_data(ctx->socket,
  ...
  /* Read ciphertext */
  if (WIFI_STATUS_OK !=
      wifi_wrapper_receive_data(ctx->socket, ciphertext_buff, ciphertext_len)) {
    return SEC_COMM_ERROR; // failed to read encrypted packet
  }

  /* Read auth tag */
  if (WIFI_STATUS_OK !=
      wifi_wrapper_receive_data(ctx->socket, auth_tag_buff, AUTH_TAG_LEN)) {
    return SEC_COMM_ERROR; // failed to read auth tag
  }

  /* Read nonce */
  if (WIFI_STATUS_OK !=
      wifi_wrapper_receive_data(ctx->socket, nonce_buff, NONCE_LEN)) {
    return SEC_COMM_ERROR; // failed to read nonce
  }

  /* Decrypt and store packet */
  if (0 != mbedtls_chachapoly_auth_decrypt(
               &ctx->ccp_ctx, ciphertext_len, nonce_buff, nonce_buff, NONCE_LEN,
               auth_tag_buff, ciphertext_buff, ctx->plaintext_buff))
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    if (HAL_RNG_GenerateRandomNumber(&sec_comm_hrng, &int_out) == HAL_OK) {
      for (uint32_t j = 0; j < 32;
           j += 8) { // split int_out into 4 separate bytes
        if (i < len) {
