...
// Maximum number of objects that can be stored in the flash memory (for each
// TA)
 ...
 * The stored data are organized in objects, each object has an ID, a length,
and the data
 *
 ...
 *
 * The actual data stored in the FLASH memory for persistent objects are encoded
in base64
 * The motivation behind this choice was to avoid character encoding issues as C
can treat
 * characters diffently based on the type they are declared (e.g. char, unsigned
char, int, uint, ecc.)
 *
 * In this way, the user can store trasparently the data in the FLASH memory
without worrying about the
 * character encoding and their type
 *
 * Basic encoding and decoding functions are provided in this file without the
...
// Encodes the input to the output according to the BASE64 encoding. Output_len
// contains also the termination character
 ...
 * @param total_size relevant area total flash sizewhich is dedicated for
 * relevant TA
 */
 ...
 *
 * @warning if this functions returns different than 0 and -1 please chech the
 * HAL FLASH ERR flags
 *
 * @return if it is success, returns 0, else different from 0. -1 means err for
 * erasing flash
 *
 */
static int flash_writeInternal(const char *ctx, int len, uint32_t ta_id) {
  ...
  for (i = 0; i < len; i += 8) {
    ...
    if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, start_addr + i, temp) !=
        HAL_OK) {
      HAL_FLASH_Lock();
...
/**
 * @brief Computes the expected length of the content of the object identified
 * by obj_id.
 *
...
/**
 * @brief Read the flash memory area, identify the object with the given id and
 * return its data
 *
 * @param ctx The JSON content
 * @param len Length of the content
 ...
 * @param out_len It must be given as a buffer size. If the buffer size and
 * out_len value is same, value of out_len will not change, otherwise it will
 * set 0
 *
...
int flash_readObject(const char *ctx, uint32_t len, int obj_id, char *out_buff,
  ...
  for (int i = 0; i < FLASH_MAX_OBJECT; ++i) {
    ...
    // If out_buff is NULL, that means function is using to open the object, not
    // reading
    if (!out_buff) {
      cJSON_Delete(p);
      return id->valueint;
    ...
    ret_val = base64_decode((const unsigned char *)data->valuestring,
                            strlen(data->valuestring),
                            (unsigned char *)out_buff, out_len);
...
/**
 * @brief Write new object identified with obj_id to the flash memory area
 * indicated by ta_id
 *
 ...
 * @param base_addr It indicates to starting flash addres for secure storage
 * which is relevant to id of TA
 *
...
int flash_writeNewObject(const char *ctx, uint32_t len, int obj_id,
  ...
  cJSON *p = cJSON_ParseWithLength((const char *)flash_content,
...
uint32_t flash_getFreeSize(uint32_t ta_id) {
  ...
  for (uint32_t i = 0; i < total_size; i += CALC_FREE_SIZE_OFF) {
    ...
    if (offset != -1) {
      free_size =
          total_size - (offset + i + 2); // +2 for the some extra character end
                                         // of the content like } or etc.
      break;
    }
  }
...
int flash_deleteObject(uint32_t ta_id, int obj_id) {
  ...
  for (index = 0; index <= 32; ++index) {
    ...
    if (!id)
      goto end;
    // check if the id is the same. If yes, the desired object object is found
    // and break the loop
    if (id->valueint != obj_id)
      continue;
