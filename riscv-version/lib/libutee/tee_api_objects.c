...
__TEE_ObjectHandle persistent_obj;
				       ...
				       TEE_ObjectHandle *object)
{
    ...
    if(!obj->buffer){
        free(obj);
        EMSG("Buffer allocation for transient object failed");
        return TEE_HANDLE_NULL;
    }
...
{
	if(!object){
        EMSG("Object does not exists");
        return TEE_ERROR_BAD_PARAMETERS;
    }

    if(!attrs){
        EMSG("Attributes does not exists");
        return TEE_ERROR_BAD_PARAMETERS;
    }
				      ...
				      TEE_ObjectHandle *object)
{
    ...
    {
        EMSG("Initial data param err");
        return TEE_ERROR_BAD_PARAMETERS;
    }
	
    memcpy(&persistent_obj.obj_id, objectID, objectIDLen);

    //Allocate memory to store initial Data
    persistent_obj.buffer = (char*)malloc(initialDataLen * sizeof(char));
    memcpy(persistent_obj.buffer, initialData, initialDataLen);
    persistent_obj.len = initialDataLen;

    // Set the object flags
    persistent_obj.flags = flags;

    // Set the object storage type
    persistent_obj.obj_storage_type = 0x00;
...
TEE_Result TEE_CloseAndDeletePersistentObject1(TEE_ObjectHandle object)
{
	...
	memset(temp_obj, 0, sizeof(__TEE_ObjectHandle));
